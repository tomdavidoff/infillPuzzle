...
ggplot(dtUsePPSF, aes(x = ppsfBin, y = share, fill = use)) +
  geom_bar(stat = "identity", position = "fill") +
  theme_minimal() +
  labs(
    title = "Vancouver Residential Permits by Price per Square Foot",
...
dtTractMean <- dtMerge[use != laneway, .(PPSF = mean(meanPPSF), elasticity = mean(elasticity), lanewaySingleShare = mean(use[use %in% c("singleOnly", "singleLaneway")] == "singleLaneway"), duplexShare = mean(use == "duplex"), multiplexShare = mean(use[permitYear > 2023] == "multi"),medianIncome=mean(medianIncome)), by = "tract"]
dtTractMean <- dtTractMean[!is.na(elasticity) & !is.na(multiplexShare)]
print(summary(dtTractMean))
mCor <- cor(dtTractMean[, .(PPSF, elasticity, lanewaySingleShare, duplexShare, multiplexShare,medianIncome)])
print(xtable(mCor, digits = 2), floating = FALSE, file = "text/tractCorrelationVancouver.tex")
# same, but neighbourhoodDescription
dtPriceND <- fread("~/OneDrive - UBC/dataProcessed/bca19_mean_ppsf_slope_by_neighbourhood.csv")
dtNeighbourhoodMean <- dtMerge[use != laneway, .( lanewaySingleShare = mean(use[use %in% c("singleOnly", "singleLaneway")] == "singleLaneway"), duplexShare = mean(use == "duplex"), multiplexShare = mean(use[permitYear > 2023] == "multi"),medianIncome=mean(medianIncome)), by = "neighbourhoodDescription"]
dtNeighbourhoodMean <- merge(dtNeighbourhoodMean, dtPriceND, by = "neighbourhoodDescription")
setnames(dtNeighbourhoodMean, old = "meanPPSF", new = "PPSF")
dtNeighbourhoodMean <- dtNeighbourhoodMean[!is.na(elasticity) & !is.na(multiplexShare)]
print(summary(dtNeighbourhoodMean))
mCor <- cor(dtNeighbourhoodMean[, .(PPSF, elasticity, lanewaySingleShare, duplexShare, multiplexShare, medianIncome)])
...
ggplot(dtSingle, aes(x = meanPPSF, y = elasticity, color = laneway)) +
  geom_point() +
  theme_minimal()
...
ggplot(dtPlexShare, aes(x = ppsfBin, y = share)) +
  geom_bar(stat = "identity", position = "dodge") +
  theme_minimal() +
  labs(
    title = "Vancouver Duplex/Multiplex Permits by Price per Square Foot",
    x = "Mean Price per Square Foot Bin",
...
cat(
  "weighted cor(predicted, actual) =",
  wcor(
    dtTractFit$pred_laneway,
...
dtOOS <- dtMerge[
  !use %chin% c("singleOnly", "singleLaneway") &
...
cat(
  "weighted cor(betaTract, duplexShare_oos) =",
