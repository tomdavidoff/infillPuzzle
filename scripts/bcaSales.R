...
residPlot <- function(yearList) {
	dmap <- df[propertyType %in% c("single","duplex","TH") & age<MAXAGE & saleYear %in% yearList]
...
baseRegTable <- data.table(
  ...
  Adjusted_R2 = sapply(list(r0,rD, rG,rL, rLN, rLT), function(reg) round(as.numeric(fitstat(reg, "ar2")),2))
)
...
print(etable(rDL,rD,tex=TRUE,file="text/vancouverEastPPSF.tex"),append=FALSE)
WIDTH <- 33
WIDEWIDTH <- 50
DEPTH <- 110
...
playData[,singlePrice:=max(totalPrice*(type=="single")),by=east]
playData[type %chin% c("duplex","quadplex"),diffSingle:=totalPrice-singlePrice]
print(xtable(playData[,.(type,east,sqft,fittedPPSF,units,totalPrice,diffSingle)]), file="text/vancouverFittedValues.tex",include.rownames=FALSE,floating=FALSE)
