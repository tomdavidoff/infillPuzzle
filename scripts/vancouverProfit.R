...
if (!file.exists(fGeo)) {
  ...
  dfG <- st_read(
    ...
    query = "SELECT ROLL_NUMBER, ACTUAL_USE_DESCRIPTION, NEIGHBOURHOOD, geom
...
ggplot(dtGeo, aes(x = longitude, y = latitude)) +
  geom_point(aes(color = NEIGHBOURHOOD))
ggsave("text/checkGEO.png")
...
# merge income by tract into a neighbourhood mean
...
dtNeighbourhoodIncome <- merge(dtGeo, dtIncome[, .(tract, medianIncome)], by = "tract")
print(dtNeighbourhoodIncome)
dtNeighbourhoodIncome <- dtNeighbourhoodIncome[, .(
  neighMedianIncome = mean(medianIncome, na.rm = TRUE)
), by = "NEIGHBOURHOOD"]
print(dtNeighbourhoodIncome)
# Replace " - " and " & " with "/"
dtNeighbourhoodIncome[, NEIGHBOURHOOD := str_to_upper(gsub(" - ", "/", NEIGHBOURHOOD))]
dtNeighbourhoodIncome[, NEIGHBOURHOOD := str_to_upper(gsub(" & ", "/", NEIGHBOURHOOD))]

# =============================================================================
# 2. SALES, INVENTORY, and DESRIPTION. NOT DONE -- see bcaSales.R
# =============================================================================
# =============================================================================
...
dtPriceND <- dtPriceND[!is.na(elasticity)] # "MARINE DRIVE"
...
dtSingle <- dtChoice[use == "single", .(neighbourhoodDescription, landWidth, landDepth, distToLaneway)]
dtSingle[, laneway := distToLaneway <= CUTDISTANCE]
print(head(dtSingle))
print(head(dtPriceND))
dtSingle <- merge(dtSingle, dtPriceND, by = "neighbourhoodDescription")
dtSingle <- merge(dtSingle, dtNeighbourhoodIncome, by.x = "neighbourhoodDescription", by.y = "NEIGHBOURHOOD")
dtLaneway <- dtSingle[, .(laneway = mean(laneway), ppsf = mean(neighPPSF), elasticity = mean(elasticity), medianIncome = mean(neighMedianIncome)), by = neighbourhoodDescription]
dtLaneway[, neighbourhoodEffect := log(laneway)]

dtPlex <- dtChoice[use != "laneway", .(plex = mean(use != "single")), by = neighbourhoodDescription]
print(dtPlex)
dtPlex <- merge(dtPlex, dtNeighbourhoodIncome, by.x = "neighbourhoodDescription")
print(dtPlex)
dfddfdf
