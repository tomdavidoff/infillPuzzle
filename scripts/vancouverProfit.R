...
dtTractMean <- dtMerge[use != laneway, .(PPSF = mean(meanPPSF), elasticity = mean(elasticity), lanewaySingleShare = mean(use[use %in% c("singleOnly", "singleLaneway")] == "singleLaneway"), duplexShare = mean(use == "duplex"), multiplexShare = mean(use[permitYear > 2023] == "multi")), by = "tract"]
dtTractMean <- dtTractMean[!is.na(elasticity) & !is.na(multiplexShare)]
print(summary(dtTractMean))
mCor <- cor(dtTractMean[, .(PPSF, elasticity, lanewaySingleShare, duplexShare, multiplexShare)])
print(xtable(mCor, digits = 2), floating = FALSE, file = "text/tractCorrelationVancouver.tex")
# same, but neighbourhoodDescription
dtNeighbourhoodMean <- dtMerge[use != laneway, .(PPSF = mean(meanPPSF), elasticity = mean(elasticity), lanewaySingleShare = mean(use[use %in% c("singleOnly", "singleLaneway")] == "singleLaneway"), duplexShare = mean(use == "duplex"), multiplexShare = mean(use[permitYear > 2023] == "multi")), by = "neighbourhoodDescription"]
dtNeighbourhoodMean <- dtNeighbourhoodMean[!is.na(elasticity) & !is.na(multiplexShare)]
print(summary(dtNeighbourhoodMean))
mCor <- cor(dtNeighbourhoodMean[, .(PPSF, elasticity, lanewaySingleShare, duplexShare, multiplexShare)])
