...
\begin{frame}
	\frametitle{Roadmap}
	\begin{itemize}
		\item Budding literature on infill upzoning takeup sorting
		\item Model of sorting with relaxation of initial zoning
		\item Pure ``division'' duplex zoning in Vancouver
		\item Mixed empirical support for the model
		\item A few words about Minneapolis and Portland
	\end{itemize}
\end{frame}
...
\begin{frame}
	\frametitle{Theoretical claim}
	\begin{itemize}
		\item Plausible that
			\begin{itemize}
				\item Highest types sort into best (highest price/sqft) locations
				\item Value of \emph{adding} space greatest there
				\item Value of \emph{dividing} space greater in low-value locations
			\end{itemize}
		\item Relates to monocentric city horse race:
			\begin{itemize}
				\item Gradient of rent in distance: $\frac{\text{marginal transport cost}}{\text{housing consumption}}$
				\item Numerator (maybe) and denominator greater for high types
				\item With fixed housing consumption, denominator effect disappears
				\item So if infill zoning modest, we may expect persistent + sorting
			\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize} 
		\item $U\left(\underbrace{y-p\left(\theta,\bar{S},{N}\right)}_{\text{numeraire}},\underbrace{\theta}_{\text{location}},\underbrace{\frac{\bar{S}}{N}}_{\text{size}},N\right)$
		\item Single crossing in $\theta$ with enough separability
		\item Standard urban model: choice over size complicates this
		\item For $S$, $N$ fixed by zoning, rich get better locations
		\item Open city, boundary $\theta^{*}$ separates high/low $y$.
		\item Higher $\theta$ is better location
		\item $\theta^{*}$ rises with $N$ 
		\item $\theta^{*}$ falls with $S$
		\item Assuming both types are in the city
	\end{itemize}
\end{frame}

\begin{frame}
	\frametitle{Theoretical ambiguity in a graph}
	\pgfimage[width=\textwidth]{bidRentModel.png}
\end{frame}


\begin{frame}
	\frametitle{City of Vancouver unit regressions by type: zoning binds!\\Single, Duplex, condo (unconstrained!)}
	\begin{footnotesize}
...
\begin{frame}
	\frametitle{Econometric issues}
	\begin{itemize}
		\item We would like to do a horse race:
			\begin{itemize}
				\item Reg lot-level infill takeup on local price, + (puzzle)
				\item Add local price elasticity, price coef $\rightarrow 0$ (resolution)
			\end{itemize}
		\item Excuse for coming results: 
			\begin{itemize}
				\item Local price level better estimated than elasticity
				\item Cor(level, elasticity) in Greater Vancouver 
					\begin{itemize}
						\item .25 at nbhd level unweighted
						\item .83 at jurisidction level weighted
					\end{itemize}
				\item Plausible to us that industry would act as if identical
			\end{itemize}
	\end{itemize}
\end{frame}

\begin{frame}
	\frametitle{Duplex-era examples: Dunbar  vs less rich Hastings-Sunrise}
	\pgfimage[width=1.2\textwidth]{dunbarHastings}
\end{frame}

\begin{frame}
	\frametitle{Geography of price levels, Vancouver}
	 Left: spatially weighted transaction residuals 2014-2018, source BC Assessment at CoV permit locations (Vancouver open data). Right: lot-level BC Assessment estimates of land value per lot square foot \\
	 \pgfimage[width=.475\textwidth]{surface_LocalPPSF.png}
	\pgfimage[width=.475\textwidth]{surface_bcaLogLandPPSF.png}
\end{frame}

\begin{frame}
	\frametitle{Geography of elasticity, Vancouver\\ Price penalty among homes on 33 foot lots for duplex vs single}
...
\begin{frame}
	\frametitle{2x2x2 model of housing choice and sorting}
	\begin{itemize}
		\item Types of households: high/low income 
		\item Types of locations $\theta$: East (worse), West better
		\item Types of homes $s$: duplex (worse), house better
		\item $U(y,\theta,s) = \underbrace{\text{concave}}u(y-p(\theta,s)) + A_{\theta}(\theta) + A_s(s)$
		\item Open city, outside option $U_(0)(y) = u(y-x) + A_{0}$
		\item Specify $u(c) = log(c)$ for starters
	\end{itemize}
\end{frame}

\begin{frame}
	\frametitle{Some features of equilibrium}
	\begin{itemize}
		\item Set zero amenity to East and Duplex
		\item By open city $U(\text{East,Duplex}) = \log(y-p(E,D)) = \log(y-x) + A_0$
		\item $p(E,D) = y - (y-x)e^{A_0}$
		\item $p(E,H) = y - (y-x)e^{A_0 - A_s}$
		\item $p(W,D) = y - (y-x)e^{A_0 - A_{\theta}}$
		\item $p(W,H) = y - (y-x)e^{A_0 - A_{\theta} - A_s}$
		\item $\left[p(W,H)-p(W,D)\right]-\left[p(E,H)-p(E,D)\right] = (y-x)e^{A_0}\left(e^{-A_{\theta}} - 1\right) < 0$
		\item Amenity and size are normal
		\item Low types outbidding for East, need $A_{\theta} > A_s$, $A_{0}$, $x$ large
		\item Duplex split more profitable East requires:
			\begin{itemize}
				\item $2p(E,D) - p(E,H)-c > 2p(W,D) - p(W,H)-c$
				\item $c$ a construction cost
			\end{itemize}
	\end{itemize}
\end{frame}

\begin{frame}
	\frametitle{Contrived example that ``works''}
	\begin{tabular}{lcc}
		\hline
		& Low Type & High Type \\
		\hline\hline
...
\begin{frame}
	\frametitle{Initial equilibrium all high, highs outbid for splits}
	\begin{tabular}{lcc}
		\hline
		& Low Type & High Type \\
		\hline\hline
